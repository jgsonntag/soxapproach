...
#define fineneedle 500.0 // needle appears at this many feet xtd

#define gsneedle 500.0 // needle appears at this many feet above/below glideslope
...
void Mapview::paintEvent(QPaintEvent *)
{
  ...
  // Draw the XTD (localizer) needles
  if (fabs(current->getxtd())>50.0)
    paint->setPen(QPen(red,6,QPen::SolidLine));
  else if (fabs(current->getxtd())>20.0)
    paint->setPen(QPen(yellow,6,QPen::SolidLine));
  else
    paint->setPen(QPen(green,6,QPen::SolidLine));
  ...
  // Draw the glideslope needles
  printf("abs value of gs: %lf\n",fabs(current->getglideslope()));
  if (fabs(current->getglideslope())>50.0)
    paint->setPen(QPen(red,6,QPen::SolidLine));
  else if (fabs(current->getglideslope())>20.0)
    paint->setPen(QPen(yellow,6,QPen::SolidLine));
  else
    paint->setPen(QPen(green,6,QPen::SolidLine));
  yneedle = int(0.5*height()*(1.0+current->getglideslope()/gsneedle));
  if (yneedle<0) yneedle=0;
  if (yneedle>height()) yneedle=height();
  paint->drawLine(0,yneedle,width(),yneedle);

  // Draw the ILS graticule
  if (fabs(current->getglideslope())>50.0)
    paint->setPen(QPen(red,1,QPen::DotLine));
  else if (fabs(current->getglideslope())>20.0)
    paint->setPen(QPen(yellow,1,QPen::DotLine));
  else
    paint->setPen(QPen(green,1,QPen::DotLine));
  paint->drawLine(0,int(0.5*height()),width(),int(0.5*height()));
  if (fabs(current->getxtd())>50.0)
    paint->setPen(QPen(red,1,QPen::DotLine));
  else if (fabs(current->getxtd())>20.0)
    paint->setPen(QPen(yellow,1,QPen::DotLine));
    ...
    paint->setPen(QPen(green,1,QPen::DotLine));
  paint->drawLine(int(0.5*width()),0,int(0.5*width()),height());
  ...
  stemp.sprintf("%.2lf ms redraw\n%5.1lf cent mer\n%6.2lf deg distortion\n%d gain L\n%d gain R",
                tdiff/1000.0,LON0,distort,lgain,rgain);
...
void Mapview::drawturn()
{
  ...
  geod2utm(latcenter/DEG2RAD,loncenter/DEG2RAD,&ycenter,&xcenter,&zonelead);
...
void Mapview::drawtd()
{
  ...
  for (i=0; i<2; i++)
  {
    //printf("in drawtd, i=%d %lf %lf\n",i,tdlat[i],tdlon[i]);
    dx = tdx[i]-current->getx();
    dy = tdy[i]-current->gety();
    if (trackup)
    {
      dx1 =  ctheta*dx+stheta*dy;
      dy1 = -stheta*dx+ctheta*dy;
    ...
    //wpfont.setPixelSize(16);
    //paint->setFont(wpfont);
    //paint->drawText(xpix+5,ypix-5,tdlabel[i]);
    paint->setPen(QPen(waypointcolor,1));
    paint->setBrush(QBrush(waypointcolor,SolidPattern));
    //if (i==(curwp+1))
    //{
